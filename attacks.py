...
def display_types(URL, method, headers):
    ...
    r = requester(URL, method, headers, payload)
    if r != None:
        schema = r.json()
...
def dump_schema(URL, method, headers, graphversion):
    """
...
def exec_graphql(URL, method, headers, query, only_length=0):
    r = requester(URL, method, headers, query)
    try:
        graphql = r.json()
...
def exec_advanced(URL, method, headers, query):
    print(query)
    ...
    # Otherwise execute the query and display the JSON result
        ...
        print(exec_graphql(URL, method, headers, query))
...
def blind_postgresql(URL, method, headers):
    ...
    r = requester(URL, method, headers, injected)
